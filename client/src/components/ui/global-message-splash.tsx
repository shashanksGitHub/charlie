...
interface GlobalMessageSplashProps {
  currentUser: {
    fullName: string;
    photoUrl?: string;
  };
...
export default function GlobalMessageSplash({
  currentUser,
  targetUser,
  onComplete,
  duration = 3000,
}: GlobalMessageSplashProps) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div className="relative z-10 text-center text-white px-8 max-w-md">
        ...
        <div className="relative flex items-center justify-center mb-12">
          ...
          >
            <motion.div
              className="h-1 bg-gradient-to-r from-white/60 to-pink-300/60 flex-1 rounded"
              style={{ width: "80px" }}
              animate={{
                opacity: [0.6, 1, 0.6],
                scale: [1, 1.05, 1],
            ...
            >
              <Heart className="w-6 h-6 text-pink-300 fill-current drop-shadow-lg" />
            </motion.div>
            <motion.div
              className="h-1 bg-gradient-to-r from-pink-300/60 to-white/60 flex-1 rounded"
              style={{ width: "80px" }}
              animate={{
                opacity: [0.6, 1, 0.6],
                scale: [1, 1.05, 1],
        ...
        >
          ...
          <p className="text-white/95 text-xl leading-relaxed">
            ...
            <span className="font-semibold text-pink-200 drop-shadow">{targetUser.fullName}</span>
          </p>
        </motion.div>
        ...
        >
          <motion.div
            className="h-full bg-gradient-to-r from-pink-300 via-white to-pink-300 rounded-full shadow-lg"
            initial={{ width: "0%" }}
            animate={{ width: "100%" }}
            transition={{
      ...
      <div className="absolute inset-0 pointer-events-none">
        {Array.from({ length: 12 }).map((_, i) => (
          ...
          >
            <Sparkles className="w-5 h-5 text-white/40 drop-shadow" />
          </motion.div>
        ))}
      </div>
    </div>
