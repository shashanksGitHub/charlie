...
export function CityInput({ 
  ...
  const [googleSuggestions, setGoogleSuggestions] = useState<GooglePlacesSuggestion[]>([]);
  ...
  const searchGooglePlaces = async (query: string) => {
    try {
      ...
      if (data.status === 'OK' && data.predictions) {
        console.log(`[GooglePlaces] Found ${data.predictions.length} suggestions for "${query}" via backend proxy`);
        return data.predictions; // Already limited to 5 on backend
      } else {
        console.log(`[GooglePlaces] No results from backend proxy for "${query}", status: ${data.status}`);
        return [];
      }
    } catch (error) {
      console.error("[GooglePlaces] Error searching locations via backend proxy:", error);
      return [];
    } finally {
      setIsLoading(false);
    }
  ...
  const isLocalSuggestionDuplicate = (localCity: City, googleSuggestions: GooglePlacesSuggestion[]): boolean => {
    ...
    return googleSuggestions.some(googleSuggestion => {
      ...
      const googleMainText = googleSuggestion.structured_formatting.main_text.toLowerCase().trim();
      const googleSecondaryText = googleSuggestion.structured_formatting.secondary_text.toLowerCase().trim();
  ...
  const getUniqueLocalSuggestions = (localSuggestions: City[], googleSuggestions: GooglePlacesSuggestion[]): City[] => {
    return localSuggestions.filter(localCity => 
      !isLocalSuggestionDuplicate(localCity, googleSuggestions)
    );
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length >= 2) {
      ...
      debounceTimeoutRef.current = setTimeout(async () => {
        ...
        const uniqueLocalSuggestions = getUniqueLocalSuggestions(cityMatches, googleResults);
        
        // Debug logging to verify duplicate filtering
        if (cityMatches.length > uniqueLocalSuggestions.length) {
          console.log(`[CITY-INPUT] Filtered ${cityMatches.length - uniqueLocalSuggestions.length} duplicate local suggestions`);
          console.log(`[CITY-INPUT] Google results:`, googleResults.map((g: GooglePlacesSuggestion) => g.description));
          console.log(`[CITY-INPUT] Filtered out:`, cityMatches.filter(city => 
            !uniqueLocalSuggestions.some(unique => unique.city === city.city && unique.country === city.country)
      ...
      case "Enter":
        if (focusedIndex >= 0 && focusedIndex < totalSuggestions) {
          ...
          if (focusedIndex < googleSuggestions.length) {
            ...
            handleSuggestionClick(suggestions[focusedIndex - googleSuggestions.length]);
          }
          e.preventDefault();
        }
        break;
    ...
    <div className="relative">
      ...
      {showSuggestions && (
        <div 
          ...
          style={{
            boxShadow: "0 10px 25px -5px rgba(124, 58, 237, 0.15), 0 8px 10px -6px rgba(124, 58, 237, 0.1)"
          }}
        >
          {/* Loading indicator */}
          {isLoading && (
              ...
              {googleSuggestions.map((suggestion, index) => (
                <li
                  ...
                  className={cn(
                    ...
                    focusedIndex === index && "bg-purple-50 dark:bg-purple-900/30"
                  )}
                  onClick={() => handleGoogleSuggestionClick(suggestion)}
                >
                  <MapPin className="h-3.5 w-3.5 mr-2 text-emerald-500 flex-shrink-0" />
                  <div>
                    <span className="font-medium text-gray-800 dark:text-gray-200">{suggestion.structured_formatting.main_text}</span>
                    <span className="text-gray-500 dark:text-gray-400">, {suggestion.structured_formatting.secondary_text}</span>
                  </div>
                </li>
              ))}
            </ul>
            ...
            <ul className="py-1">
              {suggestions.map((suggestion, index) => (
                <li
                  ...
                  className={cn(
                    "px-4 py-2.5 cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/30 text-sm flex items-center transition-colors duration-150",
                    focusedIndex === (googleSuggestions.length + index) && "bg-purple-50 dark:bg-purple-900/30"
                  )}
                  onClick={() => handleSuggestionClick(suggestion)}
                >
                  <MapPin className="h-3.5 w-3.5 mr-2 text-purple-500 flex-shrink-0" />
                  <div>
                    <span className="font-medium text-purple-800 dark:text-purple-300">{suggestion.city}</span>
                    <span className="text-purple-400 dark:text-purple-400">, {suggestion.country}</span>
                  </div>
                </li>
              ))}
            </ul>
          ...
          {/* No results found */}
          {!isLoading && googleSuggestions.length === 0 && suggestions.length === 0 && (
            <div className="px-4 py-3 text-sm text-purple-600 dark:text-purple-400 flex items-center">
              <MapPin className="h-3.5 w-3.5 mr-2 text-purple-400 flex-shrink-0" />
              <p>No cities found. Try another name or enter manually as <span className="font-semibold">"City, Country"</span>.</p>
            </div>
          )}
        </div>
      )}
