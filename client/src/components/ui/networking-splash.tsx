...
import { MessageCircle, Users, Network, Handshake, Globe, Zap } from "lucide-react";

interface NetworkingSplashProps {
  currentUser: {
    fullName: string;
    photoUrl?: string;
  };
...
export default function NetworkingSplash({
  currentUser,
  targetUser,
  onComplete,
  duration = 3000,
}: NetworkingSplashProps) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div className="relative z-10 text-center text-white px-8 max-w-md">
        ...
        <div className="relative flex items-center justify-center mb-12">
          ...
          >
            <motion.div
              className="h-1 bg-gradient-to-r from-white/60 to-green-300/60 flex-1 rounded"
              style={{ width: "80px" }}
              animate={{
                opacity: [0.6, 1, 0.6],
                scale: [1, 1.05, 1],
            ...
            >
              <Network className="w-6 h-6 text-green-200 drop-shadow-lg" />
            </motion.div>
            <motion.div
              className="h-1 bg-gradient-to-r from-green-300/60 to-white/60 flex-1 rounded"
              style={{ width: "80px" }}
              animate={{
                opacity: [0.6, 1, 0.6],
                scale: [1, 1.05, 1],
        ...
        >
          <h1 className="text-3xl font-bold mb-4 flex items-center justify-center gap-3">
            <Handshake className="w-7 h-7 text-green-200 drop-shadow-lg" />
            Network Connection!
            <Globe className="w-7 h-7 text-green-200 drop-shadow-lg" />
          </h1>
          <p className="text-white/95 text-xl leading-relaxed">
            Expand your network with{" "}
            <span className="font-semibold text-green-200 drop-shadow">{targetUser.fullName}</span>
          </p>
        </motion.div>
        ...
        >
          <motion.div
            className="h-full bg-gradient-to-r from-green-300 via-white to-green-300 rounded-full shadow-lg"
            initial={{ width: "0%" }}
            animate={{ width: "100%" }}
            transition={{
      ...
      <div className="absolute inset-0 pointer-events-none">
        {Array.from({ length: 8 }).map((_, i) => (
          ...
          >
            {i % 2 === 0 ? (
              <Network className="w-5 h-5 text-white/40 drop-shadow" />
            ) : (
              <Handshake className="w-5 h-5 text-white/40 drop-shadow" />
            )}
          </motion.div>
        ))}
      </div>
