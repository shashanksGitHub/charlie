...
import { MessageCircle, Users, Briefcase, Target, TrendingUp, Building } from "lucide-react";

interface JobSplashProps {
  currentUser: {
    fullName: string;
    photoUrl?: string;
  };
...
export default function JobSplash({
  currentUser,
  targetUser,
  onComplete,
  duration = 3000,
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div className="relative z-10 text-center text-white px-8 max-w-md">
        ...
        <div className="relative flex items-center justify-center mb-12">
          ...
          >
            <motion.div
              className="h-1 bg-gradient-to-r from-white/60 to-blue-300/60 flex-1 rounded"
              style={{ width: "80px" }}
              animate={{
                opacity: [0.6, 1, 0.6],
                scale: [1, 1.05, 1],
            ...
            >
              <Briefcase className="w-6 h-6 text-blue-200 drop-shadow-lg" />
            </motion.div>
            <motion.div
              className="h-1 bg-gradient-to-r from-blue-300/60 to-white/60 flex-1 rounded"
              style={{ width: "80px" }}
              animate={{
                opacity: [0.6, 1, 0.6],
                scale: [1, 1.05, 1],
        ...
        >
          <h1 className="text-3xl font-bold mb-4 flex items-center justify-center gap-3">
            <Target className="w-7 h-7 text-blue-200 drop-shadow-lg" />
            Career Connection!
            <TrendingUp className="w-7 h-7 text-blue-200 drop-shadow-lg" />
          </h1>
          <p className="text-white/95 text-xl leading-relaxed">
            Explore opportunities with{" "}
            <span className="font-semibold text-blue-200 drop-shadow">{targetUser.fullName}</span>
          </p>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              <Building className="w-5 h-5 drop-shadow" />
            </motion.div>
        ...
        >
          <motion.div
            className="h-full bg-gradient-to-r from-blue-300 via-white to-blue-300 rounded-full shadow-lg"
            initial={{ width: "0%" }}
            animate={{ width: "100%" }}
            transition={{
      ...
      <div className="absolute inset-0 pointer-events-none">
        {Array.from({ length: 8 }).map((_, i) => (
          ...
          >
            {i % 2 === 0 ? (
              <Briefcase className="w-5 h-5 text-white/40 drop-shadow" />
            ) : (
              <Building className="w-5 h-5 text-white/40 drop-shadow" />
            )}
          </motion.div>
        ))}
      </div>
